"""
Floating object v3 — self-consistent body dynamics, prof's reformulation.

Differences from v2 (per Carlos Galeano-Rios, April 2026 feedback):

  (1) Drop eq (2.15e) for w^3 at the surface.  Use the bulk heat equation
      "inclusively" at i = nz-1 with a one-sided second-order stencil for
      d^2/dz^2 (no ghost point above the surface needed).  Quoted prof:
        "we can ignore 2.15e if we use 2.11a and the heat equation at the
         surface inclusively"

  (2) Stress-free condition (2.11a) corrected with factor of 2:
        2 phi_xz + w^3_x + w^1_z = 0     (free surface, z=0)
      v2 had only phi_xz; full tangential-stress balance gives 2 phi_xz.

  (3) Body equation expressed as an integrated traction.  Two mathematically
      equivalent formulations exist for handling the no-penetration constraint
      under the body (phi_z + w^3 = v_b):

        Formulation A (Lagrange-multiplier, used here):
          - Add p_s(r) as an explicit unknown in the state vector.
          - p_s rows of the matrix enforce p_s = 0 on the free surface and
            phi_z + w^3 = v_b under the body (Lagrange-multiplier form).
          - Newton's row:  M v_t = -M/Fr + Δx Σ p_{s,j}  for j in body.
          - Pro: p_s is a direct diagnostic output of the linear solve.
          - Con: state vector is larger by n_x.

        Formulation B (explicit traction, prof's main.tex):
          - No p_s variable.
          - The constraint phi_z + w^3 = v_b is enforced as a direct
            equation row replacing the eta-evolution under the body.
          - Newton's row uses the closed-form traction
              tau_z = -phi_t - eta/Fr - (2/Re) d_z(phi_z + w^3)
            substituted explicitly into the integral.
          - Pro: smaller system.
          - Con: pressure must be reconstructed post-hoc from the solution.

      The two are equivalent under the substitution p_s == tau_z on the body
      region.  We use Formulation A for diagnostic clarity (the pressure
      field is directly accessible from the solution vector).

Other items unchanged from v2:
  - 2D Cartesian, periodic in x  (prof's symmetric BCs noted as TODO)
  - Bottom (z=-D) non-slip:  w^3 = -d_z phi,  w^1 = -d_x phi
  - Body (x in B):  eta = z_b,  phi_z + w^3 = v_b,  phi_x + w^1 = 0
  - Monolithic A x = b, factored once via splu, O(N) per-step back-sub.

Usage:
    from floating_object_v3 import FloatingObjectSolverV3, SimConfig, BodyParams
    config = SimConfig(body=BodyParams(mass_per_length_g_per_cm=0.5,
                                       initial_vz_cm_s=-10.0))
    solver = FloatingObjectSolverV3(config)
    result = solver.run()
"""
...
def build_monolithic_matrix_v3(
    nx: int,
    nz: int,
    dx: float,
    dz: float,
    ...
    """
    Assemble the full monolithic matrix A for v3.

    Row blocks (in order):
        1) w^3 bulk + BCs       : nx*nz rows
                                  - bulk i=1..nz-2: heat eq (centred d_zz)
                                  - i=0 (bottom):    non-slip implicit
                                  - i=nz-1 (surf):   heat eq with one-sided
                                                     d_zz (v3: replaces 2.15e)
        2) w^1 bulk + BCs       : nx*nz rows
                                  - i=nz-1 free j:   stress-free with factor
                                                     2 on phi_xz (v3 fix)
                                  - i=nz-1 body j:   horizontal no-slip
        3) phi (Laplace + dyn)  : nx*nz rows  (unchanged from v2)
        4) eta                  : nx rows
        5) p_s                  : nx rows  (kept for diagnostic; equals tau_z
                                            on body via dynamic BC + p_s row)
        6) z_b                  : 1 row
    ...
    for i in range(nz):
        for j in range(nx):
            ...
            if i == 0:
                ...
                # v3 CHANGE: drop 2.15e.  Use bulk heat equation "inclusively"
                #   d_t w^3 = (1/Re) Laplacian w^3
                # at the surface row, with a backward (one-sided) 2nd-order
                # stencil for d^2/dz^2:
                #   d_zz w^3 |_{nz-1} ~= (w^3_{nz-1} - 2 w^3_{nz-2} + w^3_{nz-3}) / dz^2
                # Implicit Euler then gives:
                #   (1 + 2 dt/Re cx + dt/Re cz) w^3[-1,j]
                #     - dt/Re cx (w^3[-1,j+1] + w^3[-1,j-1])
                #     + 2 dt/Re cz w^3[-2,j]
                #     - dt/Re cz w^3[-3,j]
                #   = w^3[-1,j]^n
                A[row, w3(nz - 1, j)] = 1.0 + 2.0 * dt_Re * cx + dt_Re * cz
                A[row, w3(nz - 1, jr(j))] = -dt_Re * cx
                A[row, w3(nz - 1, jl(j))] = -dt_Re * cx
                A[row, w3(nz - 2, j)] = 2.0 * dt_Re * cz
                A[row, w3(nz - 3, j)] = -dt_Re * cz
        ...
        for j in range(nx):
            ...
            if i == 0:
                ...
                if body_mask[j]:
                    ...
                    # v3 CHANGE: full stress-free (2.11a) is
                    #   2 phi_xz + w^3_x + w^1_z = 0    at z=0
                    # discretised as
                    #   (w^1[-1] - w^1[-2])/dz + Dx w^3[-1] + 2 Dx (phi_z) = 0
                    # =>  w^1[-1] - w^1[-2] + dz Dx w^3[-1] = -2 dz Dx (phi_z^n)
                    # The factor of 2 on phi_xz term was missing in v2.
                    A[row, w1(nz - 1, j)] = 1.0
                    A[row, w1(nz - 2, j)] = -1.0
...
def build_monolithic_rhs_v3(
    ...
    # ---- w^3 block ----
    # v3: surface row uses bulk heat eq inclusively, RHS is just w^3^n
    # (no explicit phi term — the 2.15e dependence on phi^n is gone).
    for i in range(nz):
        if i == 0:
            # Bottom: homogeneous (no explicit part)
            continue
        else:
            # Interior AND surface: heat equation, RHS = w^3^n
    ...
    for i in range(nz):
        if i == 0:
            ...
            # v3 CHANGE: factor of 2 on phi_xz (full stress-free).
            #   Free surface RHS: -2 dz phi_xz^n;  Body: 0
            phi_z_n = (phi_n[-1, :] - phi_n[-2, :]) / dz
            phi_xz_n = Dx.dot(phi_z_n)
            rhs_w1s = np.zeros(nx)
            rhs_w1s[~body_mask] = -2.0 * dz * phi_xz_n[~body_mask]
...
class FloatingObjectSolverV3:
    ...
    def _setup(self):
        ...
        t0 = timer.time()
        A, self.off = build_monolithic_matrix_v3(
        ...
        if c.verbose:
            print("FloatingObjectSolverV3 initialised")
...
def main():
    ...
    solver = FloatingObjectSolverV3(cfg)
    result = solver.run()
