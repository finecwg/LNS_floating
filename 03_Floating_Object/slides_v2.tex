...
\title{Floating Object Solver \texorpdfstring{v2~$\to$~v3}{v2 -> v3}}
\subtitle{Self-consistent body dynamics via a monolithic constraint solve\\
\small (slides updated for v3 corrections per Galeano-Rios's April 2026 feedback)}
\author{Wongyung}
...
\vspace{0.5em}
\textbf{Under body:} $p_s$ becomes an \alert{unknown}, determined by enforcing the no-penetration constraint $\partial_z\phi + w^3 = v_b$.
...
Horizontal $w^1$ & $2\partial_{xz}\phi + \partial_x w^3 + \partial_z w^1 = 0$ \alert{[v3 fix]} & $\partial_x\phi + w^1 = 0$ \\
...
\vspace{0.5em}
\textbf{Interpretation}: $p_s$ on $\mathcal{B}$ acts like a Lagrange multiplier — whatever pressure is required to make the fluid follow the body.
\end{frame}

% ===================================================================
\begin{frame}{Two equivalent formulations of the body coupling}
\small
The coupled fluid--body problem can be written in either of two equivalent ways:

\begin{block}{Formulation A — Lagrange multiplier (\textbf{used in v3 code})}
\begin{itemize}
\item State vector includes $p_s$ as an explicit unknown: $\xx = [w^3, w^1, \phi, \eta, \alert{p_s}, z_b, v_b]$.
\item $p_s$-rows of the matrix enforce \textit{$p_s = 0$ outside body} and \textit{$\phi_z + w^3 = v_b$ under body}.
\item Newton's row: $M\dot v_b = -M/\Fr + \int_\mathcal{B} p_s\,\dd x$.
\item \textcolor{primarycol}{Pro: $p_s$ is a direct diagnostic output.} \textcolor{accentcol}{Con: state vector has $n_x$ extra rows.}
\end{itemize}
\end{block}

\begin{block}{Formulation B — Explicit traction (prof's \texttt{main.tex})}
\begin{itemize}
\item No $p_s$ variable.
\item Constraint $\phi_z + w^3 = v_b$ enforced as a direct row replacing the $\eta$-row under body.
\item Newton's row uses the closed-form traction
\[
\tau_z = -\partial_t\phi - \tfrac{\eta}{\Fr} - \tfrac{2}{\Rey}\partial_z(\phi_z + w^3)
\]
substituted explicitly into the integral.
\item \textcolor{primarycol}{Pro: smaller system.} \textcolor{accentcol}{Con: pressure must be reconstructed post-hoc.}
\end{itemize}
\end{block}

\vspace{0.3em}
\small\textbf{Equivalence}: $p_s \equiv \tau_z$ on the body region. Both give the same physics; just different ways to spell the same coupled implicit problem.
\end{frame}
...
\begin{block}{Surface ($i = n_z-1$): heat eq inclusively \alert{[v3]}}
\[
\Big(I - \tfrac{\Delta t}{\Rey}\Delta_{2\text{D}}\Big)\, w^{3,n+1}_s \;=\; w^{3,n}_s
\]
with one-sided $\partial_{zz}$ stencil $(w^3_{-1} - 2 w^3_{-2} + w^3_{-3})/\Delta z^2$.
\vspace{0.2em}
\small \alert{v3 update}: the auxiliary surface vorticity equation (2.15e) is dropped.  Instead, the bulk heat equation is applied "inclusively" at the surface row, which gives the same physics with one fewer auxiliary equation.
\end{block}
\end{frame}
...
\begin{block}{Free node ($\chi_j = 0$): stress-free (2.11a) \alert{[v3 fix]}}
The full tangential-stress balance $\sigma_{xz} = \mu(\partial_x u_z + \partial_z u_x) = 0$ gives
\[
2\,\partial_{xz}\phi + \partial_x w^3 + \partial_z w^1 = 0,
\]
discretised as
\[
w^{1,n+1}_{-1,j} - w^{1,n+1}_{-2,j} + \Delta z\,(D_x\,w^{3,n+1}_{-1})_j \;=\; \alert{-2}\,\Delta z\,(\partial_{xz}\phi^n)_j.
\]
\small \alert{Factor of 2 on $\phi_{xz}$} (both $\partial_x u_z$ and $\partial_z u_x$ contribute via the symmetric mixed derivative).  v2 had only a single $\phi_{xz}$, which was an algebra error.
\end{block}
