...
The appeal of this decomposition is twofold.
First, the irrotational part $\nabla\phi$ satisfies Laplace's equation in the bulk---a time-independent elliptic problem that can be solved by standard sparse linear algebra once the boundary data at the free surface and the bottom are specified.
Second, the vortical part $\ww$ is governed by linear parabolic (heat) equations, which carry all of the viscous and vorticity-generating effects.
For free-surface flows at moderate-to-high Reynolds numbers, the bulk flow is dominated by the potential component, while the vortical component is predominantly concentrated in thin boundary layers near the surface and the solid bottom.
The decomposition thus isolates viscous effects into compact regions, leading to a system of linear partial differential equations whose structure is amenable to efficient time stepping.
...
\end{align}
Equation~\eqref{eq:laplace} follows from the identity $\nabla \cdot (\nabla\phi) = \Delta\phi$ combined with the imposed solenoidal property of the potential part, and it is time-independent: the potential field adjusts instantaneously to the boundary data at the surface and the bottom.
Equation~\eqref{eq:heat} follows from taking the curl of the linearised Navier--Stokes equation and using the orthogonality of $\nabla\phi$ and $\ww$; it expresses the fact that, in the bulk, the vortical field diffuses with kinematic viscosity $\nu$ and does not self-advect (a consequence of the linearisation).
The coupling between $\phi$ and $\ww$ is therefore entirely contained in the boundary conditions at $z' = 0$ and $z' = -D'$, presented next.
...
Three physical requirements must be imposed at the free surface: a \emph{kinematic} condition stating that the surface moves with the fluid, a \emph{dynamic} condition balancing normal stresses at the interface, and a \emph{vorticity} condition derived from the evolution of the tangential vortical component.
In addition, a \emph{stress-free} condition on the horizontal vortical component $w^1$ closes the system.
All four conditions are given below in their dimensional form (cf.\ \citealt{GaleanoRiosEtAl2017}, equations~2.15c--e and~2.11a):
...
\end{equation}
where $\phi'_s(x', t') = \phi'(x', 0, t')$ is the surface velocity potential, $\partial_{x'x'}$ is the horizontal Laplacian (which equals $\Delta_H$ in 2D), $p'_s$ is an externally applied surface pressure (zero for an uncovered free surface), and $\kappa[\eta']$ is the mean curvature of the interface.
Under the linearisation assumption, $\kappa[\eta'] \approx \partial_{x'x'}\eta'$.
The four right-hand terms admit a clear physical interpretation:
the first ($-g\eta'$) is the hydrostatic restoring force that drives gravity waves;
the second ($\sigma\kappa/\rho$) is the capillary restoring force from surface tension;
the third ($2\nu\,\partial_{x'x'}\phi'_s$) is the tangential viscous stress associated with the horizontal curvature of the potential at the surface;
and the fourth ($-2\nu\,\partial_{z'} w^{\prime 3}$) captures the viscous coupling between the vortical field and the surface potential via the vertical gradient of $w^{\prime 3}$.
The last term is crucial: it is the reason the surface potential and the vortical component do not decouple, even under the linearisation.
...
\subsubsection{Stress-free condition for \texorpdfstring{$w^1$}{w\textasciicircum 1} at the surface}
...
Combining \eqref{eq:surf_kin} and \eqref{eq:surf_vort} and integrating in time yields the diagnostic relation
\begin{equation}
    w^{\prime 3} \;=\; 2\nu\, \Delta_H' \eta' \quad \text{at } z' = 0,
    \label{eq:w3_diagnostic_remark}
\end{equation}
in the sense that the surface value of $w^{\prime 3}$ is determined algebraically by the surface elevation once initial vorticity transients have decayed \citep{GaleanoRiosEtAl2017}.
This simplification is attractive because it eliminates one partial differential equation from the surface system; however, its validity rests on two conditions that are not always met: first, that the initial state has no residual surface vorticity, and second, that the Reynolds number is large enough that transient vorticity dynamics are irrelevant on the timescale of interest.
...
In the present implementation we therefore retain equation~\eqref{eq:surf_vort} as an independent evolution equation for the surface $w^{\prime 3}$, solve the bulk heat equations~\eqref{eq:heat} for $w^{\prime 1}$ and $w^{\prime 3}$ throughout the domain, and impose the full non-slip bottom condition separately.
This design choice admits a wider range of initial conditions and correctly captures the short-time transient behaviour at the surface.
...
\begin{align}
    ...
    \phi_z + w^3 &= v,& \qquad z=0, x\leq L,t>0;\\
    2\phi_{xz}+w^1_z+w^3_x&=0,&z=0,x>L,t>0;\\
     \phi_x &= -w^1,& \qquad z= 0, x\leq L,t>0; \\
    \phi_x &= -w^1,& \qquad z= -H,t>0; \\
    \phi_z &= -w^3,& \qquad z= -H,t>0; \\
    h_t &= v,& \qquad t>0;\\
    v_t &= -\frac{1}{\Fr} + \frac{1}{Ma}\int_0^L{\left[-\phi_t-\frac{1}{\Fr}\eta-\frac{2}{\Rey}\left(\phi_z+w^3\right)_z\right]},& \qquad t>0.
\end{align}
